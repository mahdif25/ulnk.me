...
$stmt = $pdo->prepare('SELECT * FROM deals ORDER BY `id` DESC');
$stmt->execute();
  ...
  filter: brightness(1.10) grayscale(100%); 
			...
			<main class="content">
						...
						<div class="col-12">
							<div class="card">
								<div class="card-header">
									<h5 class="card-title mb-0">Deals List</h5>
								</div>
				...
				<tr>
					...
					<td>#</td>
                    <td>Client</td>
                    <td class="responsive-hidden">product_id</td>
                    <td class="responsive-hidden">order_id</td>
                    <td class="responsive-hidden">asin</td>
                    <td class="responsive-hidden">ip</td>
					<td class="responsive-hidden">free_product_id</td>
			...
			<?php if (!$accounts): ?>
                <tr>
                    <td colspan="8" style="text-align:center;">There are no Deal !</td>
                </tr>
                <?php endif; ?>
                ...
                <tr>
					...
					<td><?=$account['asin']?></td>
					<td><?=$account['ip_']?></td>
					<td><?=$account['free_product_id']?></td>
                    <td class="responsive-hidden"><?=date('m/d/Y H:i:s', $account['time_'])?></td>
					<td class="responsive-hidden"><?=$account['data']?></td>
